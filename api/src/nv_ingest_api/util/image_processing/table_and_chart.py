...
def match_bboxes(yolox_box, custom_ocr_boxes, already_matched=None, delta=2.0):
    """
    ...
    Args:
        ...
        custom_ocr_boxes (np array [n x 4]): PaddleOCR boxes
...
def join_yolox_graphic_elements_and_custom_ocr_output(yolox_output, custom_ocr_boxes, custom_ocr_txts):
    """
    Matching boxes
    We need to associate a text to the custom_ocr detections.
    ...
    custom_ocr_txts = np.array(custom_ocr_txts)
    custom_ocr_boxes = np.array(custom_ocr_boxes)

    if (custom_ocr_txts.size == 0) or (custom_ocr_boxes.size == 0):
        return {}

    custom_ocr_boxes = np.array(
        [
            custom_ocr_boxes[:, :, 0].min(-1),
            custom_ocr_boxes[:, :, 1].min(-1),
            custom_ocr_boxes[:, :, 0].max(-1),
            custom_ocr_boxes[:, :, 1].max(-1),
        ]
    ).T
    ...
    for k in KEPT_CLASSES:
        ...
        for yolox_box in yolox_output[k]:
            ...
            custom_ocr_ids = match_bboxes(yolox_box, custom_ocr_boxes, already_matched=already_matched, delta=4)

            if len(custom_ocr_ids) > 0:
                text = " ".join(custom_ocr_txts[custom_ocr_ids].tolist())
                texts.append(text)
...
def convert_custom_ocr_response_to_psuedo_markdown(bboxes, texts):
...
def join_yolox_table_structure_and_custom_ocr_output(yolox_cell_preds, custom_ocr_boxes, custom_ocr_txts):
    if (not custom_ocr_boxes) or (not custom_ocr_txts):
        return ""

    custom_ocr_boxes = np.array(custom_ocr_boxes)
    custom_ocr_boxes_ = np.array(
        [
            custom_ocr_boxes[:, :, 0].min(-1),
            custom_ocr_boxes[:, :, 1].min(-1),
            custom_ocr_boxes[:, :, 0].max(-1),
    ...
    assignments = []
    for i, (b, t) in enumerate(zip(custom_ocr_boxes_, custom_ocr_txts)):
        # Find a cell
    ...
    df_text = df_text.rename(columns={"custom_ocr_box": "box"})
...
def assign_boxes(custom_ocr_box, boxes, delta=2.0, min_overlap=0.25):
    """
    Assigns the closest bounding boxes to a reference `custom_ocr_box` based on overlap.

    Args:
        custom_ocr_box (list or numpy.ndarray): Reference bounding box [x_min, y_min, x_max, y_max].
    ...
    x0_1, y0_1, x1_1, y1_1 = custom_ocr_box
    x0_2, y0_2, x1_2, y1_2 = (
        boxes[:, 0],
        boxes[:, 1],
        boxes[:, 2],
    ...
    # Normalize by custom_ocr_box size
    area_1 = (y1_1 - y0_1) * (x1_1 - x0_1)
...
def merge_text_in_cell(df_cell):
    ...
    """
    custom_ocr_boxes = np.stack(df_cell["custom_ocr_box"].values)

    df_cell["x"] = (custom_ocr_boxes[:, 0] - custom_ocr_boxes[:, 0].min()) // 10
    df_cell["y"] = (custom_ocr_boxes[:, 1] - custom_ocr_boxes[:, 1].min()) // 10
    df_cell = df_cell.sort_values(["y", "x"])

    text = " ".join(df_cell["text"].values.tolist())
    df_cell["text"] = text
    df_cell = df_cell.head(1)
    df_cell["custom_ocr_box"] = df_cell["cell"]
