...
def ingest_sink_callable_signature(sig: inspect.Signature):
    """
    Validates that a callable has the signature:
        (IngestControlMessage, BaseModel) -> None
    ...
    if return_type is not None and return_type is not inspect.Signature.empty:
        raise TypeError(f"Return type must be None, got {return_type}")


def ingest_source_callable_signature(sig: inspect.Signature):
    """
    Validates that a callable has the signature:
        (BaseModel) -> IngestControlMessage
    ...
    if params[0].name != "stage_config":
        raise TypeError("Expected parameter name: 'stage_config'")

    param_type = params[0].annotation
    return_type = sig.return_annotation

    if not issubclass(param_type, BaseModel):
        raise TypeError(f"First parameter must be a subclass of BaseModel, got {param_type}")
