...
def test_valid_yolox_only():
    config = ChartExtractorConfigSchema(
        yolox_endpoints=("grpc_service", None), custom_ocr_endpoints=("grpc_custom_ocr", "http_custom_ocr")
    )
    assert config.yolox_endpoints == ("grpc_service", None)
    assert config.custom_ocr_endpoints == ("grpc_custom_ocr", "http_custom_ocr")


def test_valid_custom_ocr_only():
    config = ChartExtractorConfigSchema(
        yolox_endpoints=("grpc_service", None), custom_ocr_endpoints=(None, "http_custom_ocr")
    )
    assert config.custom_ocr_endpoints == (None, "http_custom_ocr")


def test_both_endpoints_provided():
    config = ChartExtractorConfigSchema(
        yolox_endpoints=("grpc_service", "http_service"), custom_ocr_endpoints=("grpc_custom_ocr", "http_custom_ocr")
    )
...
def test_invalid_yolox_empty():
    with pytest.raises(ValidationError) as excinfo:
        ChartExtractorConfigSchema(yolox_endpoints=(None, None), custom_ocr_endpoints=("grpc_custom_ocr", None))
    assert "Both gRPC and HTTP services cannot be empty for yolox_endpoints." in str(excinfo.value)


def test_invalid_custom_ocr_empty():
    with pytest.raises(ValidationError) as excinfo:
        ChartExtractorConfigSchema(yolox_endpoints=("grpc_service", None), custom_ocr_endpoints=("  ", '   "  '))
...
def test_extra_fields_forbidden_in_chart_extractor_config():
    with pytest.raises(ValidationError):
        ChartExtractorConfigSchema(
            ...
            custom_ocr_endpoints=("grpc_custom_ocr", None),
...
def test_valid_extractor_with_config():
    endpoint_config = ChartExtractorConfigSchema(
        yolox_endpoints=("grpc_service", None), custom_ocr_endpoints=("grpc_custom_ocr", None)
    )
    schema = ChartExtractorSchema(
