"""
Table structure detection benchmark test case.

This test case benchmarks the Nemotron table structure model for detecting
table cells, rows, and columns in table images.

Uses torch.utils.benchmark for accurate timing with automatic CUDA synchronization
and warmup handling.

Usage:
    python -m nv_ingest_harness.cli.run --case table_structure --dataset /path/to/images
...
def main(config=None, log_path: str = "test_results") -> int:
    """
    Main test entry point for table structure benchmarking.
    ...
    print("=== Table Structure Benchmark ===")
    ...
    for img_path in tqdm(image_paths, desc="Processing images", unit="table"):
        # Load image
