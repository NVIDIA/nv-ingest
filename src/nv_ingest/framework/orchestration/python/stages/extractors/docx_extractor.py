...
def _inject_validated_config(
    ...
    """
    Helper function that injects the validated_config into the configuration for DOCX extraction
    and calls extract_primitives_from_docx_internal.
    """
...
class PythonDocxExtractorStage(PythonStage):
    """
    A Python stage that extracts content from DOCX documents.
    ...
    It expects an IngestControlMessage containing a DataFrame with DOCX document data. It then:
      ...
      2. Calls the DOCX extraction logic (via _inject_validated_config) using a validated configuration.
    ...
    def on_data(self, control_message: Any) -> Any:
        ...
        """
        self._logger.debug("PythonDocxExtractorStage.on_data: Starting DOCX extraction process.")
        ...
        if hasattr(control_message, "get_metadata") and control_message.get_metadata("trace_enabled", False):
            control_message.set_metadata("docx_extractor_processed", True)

        self._logger.debug("PythonDocxExtractorStage.on_data: DOCX extraction completed.")
