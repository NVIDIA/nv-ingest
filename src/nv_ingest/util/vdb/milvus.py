...
def bulk_upload_results_to_milvus(ingest_results, collection_name: str = "nv_ingest"):
    collection_name = reformat_collection_name(collection_name)
    milvus_host = os.getenv("MILVUS_HOST", "milvus")
    milvus_port = os.getenv("MILVUS_PORT", "19530")
    milvus_endpoint = f"http://{milvus_host}:{milvus_port}"
    logger.debug("bulk_upload_results_to_milvus milvus_endpoint: {milvus_endpoint}")
    minio_url = os.getenv("MINIO_ENDPOINT", "minio:9000")

    # Connect to Milvus
    connections.connect("default", host=milvus_host, port=milvus_port)

    logger.debug(f"Milvus URL: {milvus_endpoint}")
    ...
    for file_result in ingest_results:
        ...
        for doc_element in page_results:
            ...
            if text is None:
                ...
                if len(text) >= 65535:
                    logger.error(f"TEXT IS WAY TOO LONG!!!! Length: {len(text)}")
    ...
    logger.debug(f"Bulk uploading: {remote_writer.batch_files[0]} to Milvus")
    ...
    logger.debug(f"Milvus Task_id: {task_id} of type: {type(task_id)}")
    return task_id
...
def check_bulk_upload_status(task_id: str) -> bool:
    ...
    logger.debug("Partition name:", task.partition_name)
