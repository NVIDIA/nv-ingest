...
from nemo_retriever.chart.stage import extract_chart_data_from_primitives_df
from nemo_retriever.ingest_config import load_ingest_config_file
...
from nemo_retriever.table.config import load_table_extractor_schema_from_dict
from nemo_retriever.table.stage import extract_table_data_from_primitives_df
from nemo_retriever.text_embed.config import load_text_embedding_schema_from_dict
from nemo_retriever.text_embed.stage import embed_text_from_primitives_df
...
def _build_pdf_extractor_and_task_cfg(
    *,
    ...
    """
    Mirror the config behavior from `nemo_retriever.pdf.stage` stage1 CLI.
    """
...
def _upload_embeddings_to_lancedb_driver_side(
    ...
    *,
    ...
    This mirrors the intent of `nemo_retriever.vector_store.stage` / stage6, but uploads directly
...
def run(
    ...
    # Stage1 PDF extraction (matches `retriever pdf page-elements` CLI defaults closely)
