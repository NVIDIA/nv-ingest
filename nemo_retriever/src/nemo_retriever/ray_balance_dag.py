...
def parse_recall_metrics(stdout: str) -> dict[str, float]:
    out: dict[str, float] = {}
    if "Recall metrics (matching nemo_retriever.recall.core):" not in stdout:
        return out
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run nemo_retriever.examples.batch_pipeline across variants and report throughput."
    )
    ...
    parser.add_argument(
        "--module",
        default="nemo_retriever.examples.batch_pipeline",
        help="Python module to run (default: nemo_retriever.examples.batch_pipeline).",
    )
    parser.add_argument(
        "--logs-dir",
