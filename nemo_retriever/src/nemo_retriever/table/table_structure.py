    ...
    for label_name, items in bb.items():
        ...
        for item in items:
            if not isinstance(item, dict):
                continue
            try:
                bbox = [float(item["x_min"]), float(item["y_min"]),
                        float(item["x_max"]), float(item["y_max"])]
    ...
    if use_remote and valid:
        ...
        try:
            ...
            for local_j, row_i in enumerate(valid):
                dets = _parse_nim_bounding_boxes(response_items[local_j])
                if not dets:
        ...
        if use_remote:
            ...
            try:
                ...
                for i, resp in enumerate(response_items):
                    dets = _parse_nim_bounding_boxes(resp)
                    if not dets:
...
# ---------------------------------------------------------------------------
# Combined table-structure + OCR helpers
# ---------------------------------------------------------------------------


def _ts_ocr_error_payload(*, stage: str, exc: BaseException) -> Dict[str, Any]:
    return {
        "timing": None,
        "error": {
            "stage": str(stage),
...
def _parse_local_ts_prediction(
    pred: Dict[str, Any],
    label_names: List[str] = (),
) -> List[Dict[str, Any]]:
    """Parse a local table-structure model prediction dict directly.

    The NemotronTableStructureV1 model returns float label *indices*
    (e.g. ``2.0``, ``3.0``) rather than string class names.  The
    model uses 1-based indexing (0 = background), so label ``1`` maps
    to ``label_names[0]`` ("cell"), ``2`` → ``label_names[1]`` ("row"),
    ``3`` → ``label_names[2]`` ("column").
    """
    boxes = pred.get("boxes") or pred.get("bboxes")
    labels = pred.get("labels") or pred.get("classes")
    scores = pred.get("scores") or pred.get("conf")
    ...
    for i, (box, label) in enumerate(zip(boxes, labels)):
        ...
        if isinstance(label, str):
            ...
            # Model uses 1-based indexing (0=background).
            idx = int(label) - 1
            if 0 <= idx < len(label_names):
                label_name = label_names[idx]
            else:
                label_name = f"label_{int(label)}"
        else:
        ...
        dets.append({
            "bbox_xyxy_norm": bbox,
            "label": int(label) if isinstance(label, (int, float)) else i,
...
def table_structure_ocr_page_elements(
    ...
    **kwargs: Any,
    ...
    _DEFAULT_TABLE_STRUCTURE_LABELS = ["cell", "row", "column"]
    ...
    for row in batch_df.itertuples(index=False):
        ...
        try:
            ...
            if use_remote_ts:
                crop_b64s = [_np_rgb_to_b64_png(crop_array) for _, _, crop_array in crops]
                response_items = invoke_image_inference_batches(
                    invoke_url=ts_url,
                    image_b64_list=crop_b64s,
                ...
                if len(response_items) != len(crops):
                    raise RuntimeError(
                        f"Expected {len(crops)} table-structure responses, got {len(response_items)}"
                    )
                ...
                for _, _, crop_array in crops:
                    ...
                    dets = _prediction_to_detections(pred, label_names=label_names)
                    # If _prediction_to_detections returned nothing (e.g. model
                    # returns string labels which can't be converted to int tensors),
                    # fall back to direct parsing of the model output dict.
                    if not dets and isinstance(pred, dict):
                        dets = _parse_local_ts_prediction(pred, label_names=label_names)
                    structure_results.append(dets)

            # --- Pass 3: Run OCR on all crops ---
            ocr_results: List[Any] = []
            if use_remote_ocr:
                crop_b64s_ocr = [_np_rgb_to_b64_png(crop_array) for _, _, crop_array in crops]
                ocr_response_items = invoke_image_inference_batches(
                    invoke_url=ocr_url,
                    image_b64_list=crop_b64s_ocr,
                    api_key=api_key or None,
                    timeout_s=float(request_timeout_s),
                    max_batch_size=inference_batch_size,
                    max_pool_workers=int(retry.remote_max_pool_workers),
                ...
                if len(ocr_response_items) != len(crops):
                    raise RuntimeError(
                        f"Expected {len(crops)} OCR responses, got {len(ocr_response_items)}"
                    )
    ...
    def __call__(self, batch_df: Any, **override_kwargs: Any) -> Any:
        try:
            ...
            if isinstance(batch_df, pd.DataFrame):
                out = batch_df.copy()
                payload = _ts_ocr_error_payload(stage="table_structure_actor_call", exc=e)
                n = len(out.index)
                out["table"] = [[] for _ in range(n)]
                out["table_structure_ocr_v1"] = [payload for _ in range(n)]
                return out
            return [{"table_structure_ocr_v1": _ts_ocr_error_payload(stage="table_structure_actor_call", exc=e)}]
