...
def main() -> None:
    ...
    print(f"RAY_ADDRESS env: {repr(ray_addr_env) if ray_addr_env is not None else 'unset'} -> connecting with address={address!r}")
    if not ray.is_initialized():
        try:
            ...
            print("If the comparison uses address='local' (no --ray-address), it starts an in-process cluster and this script cannot attach.", file=sys.stderr)
            sys.exit(1)
    ...
    gpu_actors = [a for a in actors if getattr(a, "required_resources", None) and (a.required_resources or {}).get("GPU", 0) > 0]
    pending = [a for a in actors if getattr(a, "state", None) == "PENDING"]
    pending_gpu = [a for a in pending if getattr(a, "required_resources", None) and (a.required_resources or {}).get("GPU", 0) > 0]
