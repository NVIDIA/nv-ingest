...
def _patch_embedders(monkeypatch):
    """Prevent real model downloads by stubbing both embedder dataclasses."""
    fake_text = MagicMock(name="LlamaNemotronEmbed1BV2Embedder")
    fake_vl = MagicMock(name="LlamaNemotronEmbedVL1BV2Embedder")

    monkeypatch.setattr(
        "nemo_retriever.model.local.llama_nemotron_embed_1b_v2_embedder.LlamaNemotronEmbed1BV2Embedder",
        fake_text,
    )
    monkeypatch.setattr(
        "nemo_retriever.model.local.llama_nemotron_embed_vl_1b_v2_embedder.LlamaNemotronEmbedVL1BV2Embedder",
        fake_vl,
    )
