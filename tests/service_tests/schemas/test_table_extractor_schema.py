...
def test_valid_config_with_grpc_only():
    config = TableExtractorConfigSchema(
        ...
        custom_ocr_endpoints=("grpc://custom_ocr_service", None),
    )
    assert config.auth_token == "valid_token"
    assert config.yolox_endpoints == ("grpc://yolox_service", None)
    assert config.custom_ocr_endpoints == ("grpc://custom_ocr_service", None)
...
def test_valid_config_with_http_only():
    config = TableExtractorConfigSchema(
        ...
        custom_ocr_endpoints=(None, "http://custom_ocr_service"),
    )
    assert config.auth_token == "valid_token"
    assert config.yolox_endpoints == (None, "http://yolox_service")
    assert config.custom_ocr_endpoints == (None, "http://custom_ocr_service")
...
def test_valid_config_with_both_services():
    config = TableExtractorConfigSchema(
        ...
        custom_ocr_endpoints=("grpc://custom_ocr_service", "http://custom_ocr_service"),
    )
    assert config.auth_token == "valid_token"
    assert config.yolox_endpoints == ("grpc://yolox_service", "http://yolox_service")
    assert config.custom_ocr_endpoints == ("grpc://custom_ocr_service", "http://custom_ocr_service")
...
def test_invalid_config_empty_endpoints():
    with pytest.raises(ValidationError) as exc_info:
        TableExtractorConfigSchema(
            yolox_endpoints=("grpc://yolox_service", "http://yolox_service"),
            custom_ocr_endpoints=(None, None),
        )
    assert "Both gRPC and HTTP services cannot be empty for custom_ocr_endpoints" in str(exc_info.value)
...
def test_cleaning_empty_strings_in_endpoints():
    config = TableExtractorConfigSchema(
        ...
        custom_ocr_endpoints=("   ", "http://custom_ocr_service"),
    )
    assert config.yolox_endpoints == ("grpc://yolox_service", None)
    assert config.custom_ocr_endpoints == (None, "http://custom_ocr_service")
    ...
    config = TableExtractorConfigSchema(
        ...
        custom_ocr_endpoints=("grpc://custom_ocr_service", ""),
    )
    assert config.yolox_endpoints == (None, "http://yolox_service")
    assert config.custom_ocr_endpoints == ("grpc://custom_ocr_service", None)
